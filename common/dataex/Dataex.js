...
const DataexApi = function(req) {
	this.req       = req;
	this.name      = 'DataexApi';
	this.appId     = sails.config.dataexApi.appId;
	this.appSecret = sails.config.dataexApi.appSecret;
	this.host      = sails.config.dataexApi.host;
...
async function post(uri, params, headers) {
	...
	try {
		ret_data = await cutil.post({
			...
			req     : this.req,
			mod     : this.name,
			debug   : sails.config.dataexApi.debug
		});
	} catch(err) {
...
DataexApi.prototype.notify = async function($channel, $data) {
    let $params = {
		appid      : this.appId,
	...
	$params.digest = buildDigest($params, this.appSecret);

    return await post.call(this, 'channel/publish-msg', $params);
};

DataexApi.prototype.notifyMulti = async function($msgs) {
    let $params = {
		appid      : this.appId,
		timestamp  : moment().format('YYYYMMDDHHmmss'),
		version    : '1.0',
    ...
    return await post.call(this, 'channel/publish-multi-msg', $params);
};


module.exports = DataexApi;
