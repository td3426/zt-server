...
async function savePercentProduct(set, acc_sets, $product_row) {
	try {
		var $row = await sails.getDatastore('factory').transaction(async (db, proceed) => {
			try {
				...
				if(parseInt(set.stat) == CONST.PRODUCT_STAT_PUBLISHED) {
					try {
						checkPercentProductForPublish($product_row);
					} catch($e) {
				...
				if(_.size(acc_sets)) {
					...
					for(var acc_set_idx in acc_sets) {
						...
						acc_set.priceType = $product_row.priceType
...
function checkPercentProductForPublish($product_row) {
	...
	if(!$product_row.pricePercent) throw '请填写分成比例';
}

module.exports = {

    addPercentProduct: async function(req, res) {
		...
		//	if(parseInt($set_row.priceType) != CONST.PRODUCT_PRICE_TYPE_PERCENT_PRICE) return res.jsonerr('套系和商品类型不匹配');
		//}
        ...
        set.priceType = CONST.PRODUCT_PRICE_TYPE_PERCENT_PRICE;
        set.price = 0;
        set.pricePercent = parseInt(cutil.getReq(req, 'price_percent')) || 0;
        ...
        try{
			...
			let $product_row = await savePercentProduct(set, acc_sets);
	...
	listPercentProduct: async function(req, res) {
		...
		var $ret_fds = ["id", "name", "sname", "styleNo", "catId", "photoRender", "pricePercent", "stat", "createdBy", "designMarketNVisited", "publishedAt"];
		...
		var $factory_product_rows = await FactoryProduct.find({
			...
			select: ['id', 'factoryCompId', 'expireAt', 'endAt', 'designProductNo', 'stat']
		});
        ...
        $comp_rows = await Comp.getComps(_.values($factory_ids), ['id', 'name', 'logo']);
		...
		_.each($product_rows, function($row){
			...
			_.each($factory_product_rows[$row.id], function($factory_product_row) {
				...
				$factory_row.factory_product_no = $factory_product_row.id;
				$factory_row.expire_at = $factory_product_row.expireAt;
				$factory_row.end_at = $factory_product_row.endAt;
				...
				if($tm - $factory_product_row.expireAt >= 0) {
					$ret_row.factory_list.end.push($factory_row);
					...
					$ret_row.factory_list.ing.push($factory_row);
				} else {
					//不会到这里来
				}
			});
	...
	detailPercentProduct: async function(req, res) {
		...
		var $ret_fds = ["id", "name", "sname", "styleNo", "catId", "designIdea", "photoRender", "photoCad", "dimension", "intro", "photoSize", "photoStory", "pricePercent", "stat", "createdBy", "designMarketNVisited", "publishedAt", "designParentProductNo"];
		var $product_rows = await DesignProduct.find({
			where: {
				or: [
					{ id: $product_id },
        ...
        $comp_rows = await Comp.getComps(_.values($factory_ids), ['id', 'name', 'logo', 'aptitude']);
		$comp_rows = cutil.indexTabByCol($comp_rows, 'id');
	...
	updatePercentProduct: async function(req, res) {
        if(!cutil.ucan(req.me.privs, CONST.PRIV_DESIGN_MANAGE_MY_PRODUCT)) return res.jsonerr('没有权限');
        if(!req.me.compId) return res.jsonerr('没有权限');
		...
		if(typeof req.param('price_percent') != 'undefined') {
			set.pricePercent = parseInt(cutil.getReq(req, 'price_percent')) || 0;
		}
        ...
        try{
			set.createdBy = req.me.id;
			set.createdByCompId = req.me.compId;
			$product_row = await savePercentProduct(set, acc_sets, $product_row);
	...
	togglePercentProduct: async function(req, res) {
		if(!cutil.ucan(req.me.privs, CONST.PRIV_DESIGN_MANAGE_MY_PRODUCT)) return res.jsonerr('没有权限');
		if(!req.me.compId) return res.jsonerr('没有权限');
	...
	delPercentProduct: async function(req, res) {
		if(!cutil.ucan(req.me.privs, CONST.PRIV_DESIGN_MANAGE_MY_PRODUCT)) return res.jsonerr('没有权限');
		if(!req.me.compId) return res.jsonerr('没有权限');
    ...
    undelPercentProduct: async function(req, res) {
		if(!cutil.ucan(req.me.privs, CONST.PRIV_DESIGN_MANAGE_MY_PRODUCT)) return res.jsonerr('没有权限');
		if(!req.me.compId) return res.jsonerr('没有权限');
	...
	terminatePercentProduct: async function(req, res) {
		...
		if($factory_product_row.designerCompId != req.me.compId) return res.jsonerr('合作商品不属于该公司');
